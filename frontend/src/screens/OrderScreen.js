// fetch order data from the backend and show it 
// on the screen
...
function OrderScreen(props) {
    ...
    useEffect(() => {
        const addPayPaScript = async () => {
            const { data } = await Axios.get('/api/config/paypal');
            const script = document.createElement('script');
            script.type = "text/javascript";
            script.src = `https://www.paypal.com/sdk/js?client-id=${data}`;
            script.async = true;
            // onload happens when src downloaded on your browser
            script.onload = () => {
                setSdkReady(true);
            }
        ...
        if (!order || successPay || (order && order._id !== orderId)) {
            ...
            if (!order.isPaid) {
                if (!window.paypal) {
                    // here we didn't loaded the paypal 
                    addPayPaScript();
                } else {
                    setSdkReady(true);
                }
